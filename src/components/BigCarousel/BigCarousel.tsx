...
const BigCarousel: React.FC<Props> = ({id, focused, focus}) => {
  ...
  const moveCarousel = (direction: boolean) => {
    if (direction) {
      setSlide(slide - 1);
      setLeftVisible(true);
      if (-slide >= patterns.length - 1) getRandomPalette();
    } else {
      setSlide(slide + 1);
      if (-slide === 1) {
  ...
  return (
    ...
    style={id==focused?{zIndex: 20}:undefined}>
      ...
      <div className="carouselWrapper d-flex col-12 flex-column">
        <div
          className="carouselInner d-flex col-12 justify-content-start"
          style={{ left: `${slide}00%` }}
        >
          {processPatterns()}
        </div>
      ...
      <div
        className="arrow right d-flex align-items-center justify-content-center"
